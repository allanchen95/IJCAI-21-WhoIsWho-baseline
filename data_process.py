...
class raw_data:
    ...
    def _load_raw_data(self):
        ...
        # test_data
        with open(paper_data_dir + "test_author_profile.json", 'r') as files:
            self.test_author_profile = json.load(files) 
        with open(paper_data_dir + "test_author_unass.json", 'r') as files:
            self.test_author_test = json.load(files) 

        # paper_info
        with open(paper_data_dir + "train_pub.json", 'r') as files:
    ...
    def generate_train_data(self, ins_num):
        ...
        for pid in sample_pid_id:
            ...
            instance.append(((name, aid, pid, neg_author_list), name2aid2pid))
    ...
    def atomic_process(self, each_ins):
        ...
        paper_pro, pos_pro, neg_pro = self.tokenizer_padding(each, name2aid2pid)            
        return (paper_pro, pos_pro, neg_pro)
    ...
    def generate_test_data(self, ins_num):
        ...
        for pid in sample_pid_id:
            ...
            candidate_authors = list(name2aid2pid[name].keys())
            if (len(candidate_authors) <= neg_sample):
                print("testing error! person num <= :", neg_sample, len(candidate_authors))
                exit()
    ...
    def tokenizer_padding(self, each, author_dict):
        ...
        paper_pro, pos_pro, neg_pro = self.get_author_encoder(name, aid, pid, neg_author_list, author_dict)
        return paper_pro, pos_pro, neg_pro
    ...
    def get_author_encoder(self, author_name, author_id, paper_id, neg_author_list, author_dict):
        ...
        return (paper_str_features, paper_inputs),\
